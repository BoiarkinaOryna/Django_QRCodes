...
from main.models import Subsciption
...
def render_create_code(request):
    ...
    user_status = False
    user = request.user
    subscription = Subsciption.objects.filter(user_id = user.id)
    for pk in range(len(subscription)):
        user_status = subscription[pk].subsciption
    ...
    if num_qr < max_qr_num:
        if request.method == 'POST':
            ...
            if user_status == "Pro" or user_status == "Standart":
